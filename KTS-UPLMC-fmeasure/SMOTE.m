    ...
    for i=find(No~=0)        
        ...
        if k>1
            for m=1:length(rn)
                minn=min_id;
                minn(minn == id1(:,m)) = [];
                rn=floor(rand(1,1)*(k-1))+1;
                id2(:,m)=minn(rn);
            end 
            weight1=rand(NumAtt,No(i));
            weight2=rand(NumAtt,No(i));
            D=repmat(T(:,i),1,No(i));      
            % for numeric attributes
            aid=find(AttVector==0);
            %D(aid,:)=D(aid,:)+weight(aid,:).*(T(aid,id)-D(aid,:)); %original smote
            D(aid,:)=D(aid,:)+weight1(aid,:).*(T(aid,id1)-D(aid,:)); %final smote
            D(aid,:)=D(aid,:)+weight2(aid,:).*(T(aid,id2)-D(aid,:));
        else
            weight1=rand(NumAtt,No(i));
            D=repmat(T(:,i),1,No(i));      
            % for numeric attributes
